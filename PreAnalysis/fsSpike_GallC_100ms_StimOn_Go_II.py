...
# File-Name:    fsSpike_GallC_100ms_StimOn_Go_II.py
# Version:      0.0.1 (see end of script for version descriptions)
...
# Locations
resultsLoc = resultsFolderI+"PreAnalysis/fsSpike_GallC_100ms_StimOn_Go_II/"
import_loc = spikesDataII+'230413/'

# Define window characteristics
anchortimes = {'TRGDLY':[-0.3,0.8,12],
               'UNFIXTG11_ON':[-0.3,0.3,7], 
               'FIXTG0_OFF':[-0.2,0,3],
               'UNFIXTG21_ON':[-0.3,0.3,7],
               'FIXTG11_OFF':[-0.2,0,3],
               'UNFIXTG31_ON':[-0.3,0.3,7], 
               'FIXTG21_OFF':[-0.2,0,3],
               'UNFIXTG41_ON':[-0.3,0.3,7],
               'FIXTG31_OFF':[-0.2,0,3]}
...
def resolve_anchortimes(anchortimes_dict, spikerow):
    ...
    for i_anchortime, i_anchorvars in anchortimes_dict.items():
        ...
        vecends = np.linspace(start=i_anchorvars[0]+0.1, stop=i_anchorvars[1]+0.1, num=i_anchorvars[2])
